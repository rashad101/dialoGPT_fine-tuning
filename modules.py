...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int,float]:
    if args.local_rank in [-1,0]:
        ...
        if (
            ...
            optimizer.load_state_dict(torch.load(os.path.join(args.model_name_or_path, "optimier.pt")))
        ...
        for _ in train_iterator:
            epochs_iterator = tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1,0])
            for step, batch in enumerate(epochs_iterator):
                ...
                if (step+1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1,0] and args.logging_steps > 0 and global_step% args.logging_steps==0:
                        ...
                        tb_writer.add_scalar("loss", (tr_loss-loggin_loss)/ args.logging_steps, global_step)
                        loggin_loss+=tr_loss
...
def main(df_trn, df_val):
    ...
    if args.should_continue:
        ...
        if len(sorted_checkpoints)==0:
            raise ValueError('Used --should_continue but no checkpoints was found in --output_dir.')
        else:
            args.model_name_or_path = sorted_checkpoints[-1]


    if (os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train and
        not args.overwrite_output_dir and not args.should_continue):
    ...
    logger.warning(f"Process rank: {args.local_rank}, device: {args.device}, n_gpu: {args.n_gpu}, distributed training: {bool(args.local_rank!=-1)}, 16-bits training: {args.fp16}")
    ...
    if args.do_train:
        ...
        logger.info("Saving model checkpoit to %s", args.output_dir)
    ...
    if args.do_eval and args.local_rank in [-1,0]:
        ...
        logging.getLogger("Evaluate the following checkpoints: %s", checkpoints)
